...
#explain why we didn't 30 cities, only 22, adjusting by starting and ending years
#why did this inconsistency show up
...
#go step by step and explain how we took the original data, homicide_data
...
city_number_homicides <- homicide_data %>%
  ...
  filter(year == "2011" | year == "2017") %>%
  pivot_wider(names_from = year, values_from = number_homicides) %>%
  mutate(homicide_difference = `2017` - `2011`) %>%
  arrange(desc(homicide_difference))
